// pages/api/cart/remove-item.js
export default async function handler(req, res) {
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' });

  try {
    const { cartId, cartVersion, lineItemId } = req.body;
    if (!cartId || typeof cartVersion !== 'number' || !lineItemId) {
    ...
    if (!ctRes.ok) return res.status(ctRes.status).json({ error: text || 'Failed to remove item' });
